    ...
    public class ProjectController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetProjectListUserHasJoined(string token)
        {
            try
            {
                ...
                var project = await _projectLogic.GetProjectListUserHasJoined(user.Data.UserId);
                if (!project.Status)
                {
        ...
        public async Task<IActionResult> GetProjectListUserHasOwner(string token)
        {
            try
            {
                ...
                var project = await _projectLogic.GetProjectListUserHasOwner(user.Data.UserId);
                if (!project.Status)
                {
                    _loggerHelper.LogDebug(project.Message);
                    return BadRequest(project);
        ...
        public async Task<IActionResult> GetProjectDetailProjectHasJoined(string token, string projectId)
        {
            try
            {
                ...
                var project = await _projectLogic.GetProjectDetailProjectHasJoined(user.Data.UserId, projectId);
                if (!project.Status)
                {
                    _loggerHelper.LogDebug(project.Message);
                    return BadRequest(project);
